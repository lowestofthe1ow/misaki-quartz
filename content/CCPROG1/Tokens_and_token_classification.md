...
A series of characters is called a [[CCICOMP/Data_representation#String|string]]. A string literal should be enclosed in double-quotes (`""`). Note that certain special characters also need the escape character backslash (`\`) inside the double-quotes, e.g., `\"` for including a double-quote character within the string.
...
As a convention, we add prefixes in our variables to easily identify their [[CCICOMP/Data_representation#Data types|data type]]. This is referred to as **Hungarian notation**.
<!--ID: 1694694364098-->
...
An operator is a symbol in C that is used to perform certain operations on values or variables. For example, the arithmetic operators: `*`, `/`, `%`, `+`, and `-` correspond to the operations: multiplication, division, modulo, addition, and subtraction, respectively. Furthermore, evaluating operators [[Operator_types,_precedence,_and_associativity#Operator precedence and associativity|follows a hierarchy of priorities]]. Note that the [[CCICOMP/Data_representation#Data types|data type]] (whether it is an integer or floating-point number) of the operands [[Float_and_integer_operations|affects]] the result of the operation. The three basic types of operators are:
